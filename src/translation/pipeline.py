...
class TranslationPipeline:
    ...
    사용 예시:
        ...
        print(ctx.english_text)  # "Hello, how is the weather today?"  # noqa
        ...
        print(final_response)  # "안녕하세요, 오늘 날씨가 좋습니다."  # noqa
    """
...
if __name__ == "__main__":
    ...
    print(f"입력: {korean_input}")  # noqa

    ctx = pipeline.to_english(korean_input)
    print(f"언어 감지: {ctx.original_lang}")  # noqa
    print(f"영어 변환: {ctx.english_text}")  # noqa
    print(f"번역됨: {ctx.is_translated}")  # noqa

    # 모델 응답 시뮬레이션
    mock_response = "Today the weather in Seoul is cold, about -2 degrees Celsius with cloudy skies."
    print(f"\n모델 응답 (영어): {mock_response}")  # noqa

    final = pipeline.from_english(mock_response, ctx)
    print(f"최종 응답 ({ctx.original_lang}): {final}")  # noqa
