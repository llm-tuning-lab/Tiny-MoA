...
class TinyMoA:
    ...
    def __init__(
        ...
        if self.enable_translation:
            try:
                ...
                console.print("[dim]🌐 Translation Pipeline 활성화[/dim]")  # noqa
            except Exception as e:
                console.print(f"[yellow]⚠️ 번역 비활성화: {e}[/yellow]")  # noqa
        ...
        console.print("[bold blue]🤖 Tiny MoA 초기화 중...[/bold blue]")  # noqa
        ...
        console.print("[bold green]✅ Tiny MoA 준비 완료![/bold green]")  # noqa

    def _load_brain(self):
        """Brain 모델 로드"""
        if self._brain is None:
            console.print("[dim]Loading Brain (LFM2.5-1.2B)...[/dim]")  # noqa
            self._brain = Brain(
    ...
    def _load_reasoner(self):
        ...
        if self._reasoner is None:
            console.print("[dim]Loading Reasoner (Falcon-R-0.6B)...[/dim]")  # noqa
    ...
    def _load_tool_caller(self):
        ...
        if self._tool_caller is None and self.enable_tools:
            try:
                ...
                console.print("[dim]Loading Tool Caller (Falcon-90M)...[/dim]")  # noqa
                self._tool_caller = ToolCaller(
                    falcon_path=self.tool_caller_path,
                    brain_model=self._brain,  # Brain으로 JSON 보정
                )
                self._tool_executor = ToolExecutor()
                console.print("[dim]✅ Tool Caller 준비 완료[/dim]")  # noqa
            except ImportError as e:
                console.print(f"[yellow]⚠️ Tool Calling 비활성화: {e}[/yellow]")  # noqa
    ...
    def _handle_tool_call(self, user_input: str, tool_hint: str = "", arg_hint: str = "", verbose: bool = True, return_raw: bool = False) -> str:
        ...
        if arg_hint and tool_hint:
            if verbose:
                console.print(f"[dim]🧠 Brain 최적화 인자 사용: {tool_hint}({arg_hint})[/dim]")  # noqa
            ...
            if tool_hint in ["search_web", "search_news", "search_wikipedia"]:
                ...
                if is_valid_cmd:
                    ...
                    if verbose:
                         console.print(f"[yellow]⚠️ Brain 생성 명령어('{arg_hint}')가 자연어 설명으로 감지되어 무시합니다. 키워드 추론을 사용합니다.[/yellow]")  # noqa
        ...
        if not tool_call:
            if self.tool_caller and self.tool_caller._falcon:
                # Falcon-90M 사용
                if verbose:
                    console.print("[dim]🔧 Tool Caller (Falcon-90M) 호출 중...[/dim]")  # noqa
                with self._model_lock:
                    tool_call = self.tool_caller.generate_tool_call(user_input)
            else:
                # 키워드 기반 폴백 (모델 없이)
                if verbose:
                    console.print("[dim]🔧 키워드 기반 Tool 추론 중...[/dim]")  # noqa
    ...
    def _execute_tool_logic(self, tool_call: dict, verbose: bool, return_raw: bool, user_input: str) -> str:
        ...
        if "error" in tool_call:
            if verbose:
                console.print(f"[yellow]⚠️ Tool 파싱 실패: {tool_call['error']}[/yellow]")  # noqa
        ...
        if verbose:
            console.print(f"[dim]🔨 Tool 실행: {tool_name}({arguments})[/dim]")  # noqa
        ...
        if result.get("success", False):
            ...
            for keyword in error_keywords:
                if keyword in raw_result:
                    if verbose:
                        console.print(f"[yellow]⚠️ Semantic Error 감지: '{keyword}' - 재시도 트리거[/yellow]")  # noqa
        ...
        if result.get("success", False):
            ...
            if "retry" not in arguments: # 무한 루프 방지
                if verbose:
                    console.print(f"[bold red]⚠️ 실행 실패: {error}. Brain에게 수정을 요청합니다...[/bold red]")  # noqa
...
Return ONLY the JSON arguments (e.g. {{"location": "Seoul"}} or {{"command": "python --version"}}). Do NOT explain."""
                ...
                try:
                    ...
                    if corrected_args_str.startswith("{"):
                        ...
                        if verbose:
                            console.print(f"[dim]🧠 Brain 수정 제안: {retry_args}[/dim]")  # noqa
                    ...
                    if verbose:
                        console.print(f"[dim]⚠️ 재시도 파싱 실패: {e}[/dim]")  # noqa
    ...
    def _process_rag_attachments(self, user_input: str, verbose: bool = True) -> tuple[str, str]:
        ...
        if verbose:
            console.print(f"[dim]📚 RAG 파일 감지: {rag_files}[/dim]")  # noqa
        ...
        if not hasattr(self, "_rag_engine") or self._rag_engine is None:
            try:
                ...
                self._rag_engine = RAGEngine()
                    ...
                    console.print(f"[red]⚠️ RAG Engine 로드 실패: {e}[/red]")  # noqa
        ...
        if self._rag_engine:
            for file_ref in rag_files:
                ...
                if Path(file_path).exists():
                    ...
                    if verbose:
                            console.print(f"[dim]🔄 문서 처리 중: {Path(file_path).name}...[/dim]")  # noqa
                    status = self._rag_engine.ingest_file(file_path)
                    if verbose:
                            console.print(f"[dim]   Result: {status}[/dim]")  # noqa
                    ...
                    if verbose:
                            console.print(f"[yellow]⚠️ 파일을 찾을 수 없음: {file_ref}[/yellow]")  # noqa

        # 입력에서 파일 참조 제거
        clean_input = re.sub(r"@\[(.*?)\]", "", user_input).strip()

        if rag_context and verbose:
            console.print(f"[dim]📄 RAG 컨텍스트 추가됨 ({len(rag_context)} chars)[/dim]")  # noqa
    ...
    def chat(self, user_input: str, rag_context: str = "", verbose: bool = True, return_raw_tool_result: bool = False) -> str:
        ...
        if len(pipeline) > 1:
            # 다중 스텝 파이프라인 실행
            if verbose:
                console.print(f"[dim]🔗 파이프라인 감지: {len(pipeline)}단계 실행[/dim]")  # noqa
                for step in pipeline:
                    console.print(f"[dim]   Step {step['step']}: {step['route']} - {step.get('description', '')}[/dim]")  # noqa
            ...
            for step in pipeline:
                ...
                if verbose:
                    console.print(f"[dim]▶ Step {step_num}: {route}[/dim]")  # noqa
        ...
        if is_complex:
            if verbose:
                console.print("[dim]🧩 복합 질문 감지: 분해 시도 중...[/dim]")  # noqa
            ...
            if len(sub_queries) > 1:
                if verbose:
                    console.print(f"[dim]🧩 분해 결과: {sub_queries}[/dim]")  # noqa
        ...
        if self.enable_translation and self._translation_pipeline:
            ...
            if translation_ctx.is_translated:
                ...
                if verbose:
                    console.print(f"[dim]🌐 번역: {translation_ctx.original_lang} → en[/dim]")  # noqa
                    console.print(f"[dim]   영어: {processed_input[:50]}...[/dim]")  # noqa

        # 1. Brain이 라우팅 결정 (영어로 된 입력 사용)
        # [Fix] RAG 컨텍스트가 있으면 Tool Calling을 방지하고 강제로 DIRECT 응답 유도
        if rag_context:
             if verbose:
                 console.print("[dim]📄 RAG 컨텍스트 존재: 강제로 DIRECT 모드 전환[/dim]")  # noqa
             route = "DIRECT"
             specialist_prompt = ""
             tool_hint = ""
        else:
        ...
        if verbose:
            console.print(f"[dim]🧠 라우팅: {route}[/dim]")  # noqa

        # 2. 라우팅에 따른 처리
        if route == "TOOL":
            # Tool Calling
            if verbose:
                console.print(f"[dim]🔧 Tool 호출: {tool_hint}[/dim]")  # noqa
            ...
            # Reasoner 호출
            if verbose:
                console.print("[dim]🤔 Reasoner 호출 중...[/dim]")  # noqa
            ...
            # Brain이 직접 응답
            if verbose:
                console.print("[dim]🧠 Brain 직접 응답...[/dim]")  # noqa
            with self._model_lock:
        ...
        if not return_raw_tool_result and isinstance(final_response, str) and translation_ctx and translation_ctx.is_translated and self._translation_pipeline:
            if verbose:
                console.print(f"[dim]🌐 번역: en → {translation_ctx.original_lang}[/dim]")  # noqa
            with self._model_lock:
                try:
        ...
        if verbose:
            ...
            if self.use_thinking and self.show_thinking and isinstance(final_response, str):
                 ...
                 console.print("[dim blue]🧠 Thinking Process Visualization Enabled (Raw Output)[/dim blue]")  # noqa
    ...
    def run_cowork_flow(self, user_goal: str, workspace_root: str = ".", use_tui: bool = True) -> str:
        ...
        try:
            ...
            try:
                ...
                console.print("\n[green]ℹ️ 작업 결과가 저장되었습니다: docs/cowork_result.md[/green]")  # noqa
            except Exception as e:
    ...
    while True:
        try:
            ...
            if user_input.lower() in ["quit", "exit", "q"]:
                console.print("[dim]👋 안녕히 가세요![/dim]")  # noqa
                break
